...
def extract_fp2_data(url, output_file='fp2_extracted.json', circuit_id=None, event_info=None):
    ...
    try:
        ...
        if event_info:
            ...
            extracted_data = {
                "raceName": f"{event_info.get('raceName', 'Unknown Race')} - PRACTICE 2".upper(),
    ...
    if h1:
        ...
        extracted_data['raceName'] = f"{event_info.get('raceName')} - PRACTICE 2".upper()
    else:
    ...
    if len(extracted_data['results']) == 0:
        print(f"No FP2 results available yet at {url} (session may not have concluded or results are not yet published). Saving session details with empty results.")
...
def get_dynamic_url(schedule_file='schedule.json', target_date=None):
    ...
    try:
        ...
        for event in schedule:
            fp2_date = event.get('schedule', {}).get('fp2', {}).get('date')
            if fp2_date and fp2_date <= target_date:
                if best_date is None or fp2_date > best_date:
                    best_date = fp2_date
                    best_match = event
        ...
        if best_match:
            ...
            url = f"https://www.formula1.com/en/results/2026/races/{event_id}/practice/2"
            print(f"Found latest FP2 session ({best_date}): {url}")
            return url, circuit_id, best_match
                
        print(f"No FP2 session found on or before: {target_date}")
...
def push_to_git(practice_num):
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    source_dir = os.path.dirname(script_dir) # e.g., .../practice2
...
if __name__ == "__main__":
    ...
    if url:
        ...
        success = extract_fp2_data(url, output_file, circuit_id=circuit_id, event_info=event_info)
