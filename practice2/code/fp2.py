...
def extract_fp2_data(url, output_file='fp2_extracted.json', circuit_id=None):
    ...
    if len(extracted_data['results']) == 0:
        print(f"No FP2 results available yet at {url} (session may not have concluded or results are not yet published). Preserving existing data.")
        return False
...
if __name__ == "__main__":
    ...
    if url:
        ...
        success = extract_fp2_data(url, output_file, circuit_id=circuit_id)
