...
def scrape_f1_qualifying(url, output_file='qualifying_results.json', circuit_id=None, event_info=None):
    ...
    try:
        ...
        if event_info:
            ...
            country_name = event_info.get('f1siteracename', 'Unknown').capitalize()
            scraped_data = {
                "country": country_name,
                "session": "QUALIFYING",
                "raceName": f"{event_info.get('raceName', 'Unknown Race')} - QUALIFYING".upper(),
    ...
    session = parts[-1].strip() if len(parts) > 1 and parts[-1].strip() else "QUALIFYING"
    ...
    if country_name == "Unknown" and event_info and event_info.get('f1siteracename'):
        country_name = event_info.get('f1siteracename').capitalize()
    ...
    if len(results) == 0:
        print(f"No qualifying results available yet at {url} (session may not have concluded or results are not yet published). Saving session details with empty results.")
...
def get_dynamic_url(schedule_file='schedule.json', target_date=None):
    ...
    try:
        ...
        # 1. Check for active race weekend having Qualifying
        for event in schedule:
            sched = event.get('schedule', {})
            qualy_info = sched.get('qualy')
            race_date = sched.get('race', {}).get('date')
            session_dates = [v.get('date') for v in sched.values() if isinstance(v, dict) and v.get('date')]
            if session_dates and race_date and qualy_info and qualy_info.get('date'):
                ...
                if min_date <= target_date <= race_date:
                    ...
                    print(f"Found active race weekend Qualifying session ({qualy_info.get('date')}): {url}")
...
if __name__ == "__main__":
    ...
    if url:
        ...
        success = scrape_f1_qualifying(url, output_file, circuit_id=circuit_id, event_info=event)
        # Push to github automatically if scraping succeeded
        if success:
            push_to_git('qualifying')
    else:
        print("No qualifying data to scrape for the current race weekend.")
