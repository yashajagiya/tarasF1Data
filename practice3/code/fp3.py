...
def extract_fp3_data(url, output_file='fp3_extracted.json', circuit_id=None, event_info=None):
    ...
    try:
        ...
        if event_info:
            ...
            extracted_data = {
                "raceName": f"{event_info.get('raceName', 'Unknown Race')} - PRACTICE 3".upper(),
    ...
    if h1:
        ...
        extracted_data['raceName'] = f"{event_info.get('raceName')} - PRACTICE 3".upper()
    else:
    ...
    if len(extracted_data['results']) == 0:
        print(f"No FP3 results available yet at {url} (session may not have concluded or results are not yet published). Saving session details with empty results.")
        ...
        for event in schedule:
            ...
            if session_dates and race_date and fp3_info and fp3_info.get('date'):
                ...
                if min_date <= target_date <= race_date:
                    ...
                    print(f"Found active race weekend FP3 session ({fp3_info.get('date')}): {url}")
...
if __name__ == "__main__":
    ...
    if url:
        ...
        success = extract_fp3_data(url, output_file, circuit_id=circuit_id, event_info=event_info)
